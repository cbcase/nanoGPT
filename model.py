...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_attn = tensor_parallel.ColumnParallelLinear(
           input_size=config.n_embd, output_size=3 * config.n_embd, bias=True,
           gather_output=False)
        # output projection
        self.c_proj = tensor_parallel.RowParallelLinear(
           input_size=config.n_embd, output_size=config.n_embd, bias=True,
           input_is_parallel=True
        )
        # regularization
        self.attn_dropout = nn.Dropout(config.dropout)
        self.resid_dropout = nn.Dropout(config.dropout)
        self.n_head_per_partition = config.n_head // mpu.get_tensor_model_parallel_world_size()
    ...
    def forward(self, x):
        ...
        C_partitioned = C // mpu.get_tensor_model_parallel_world_size()

        # calculate query, key, values for all heads in batch and move head forward to be the batch dim
        qkv, _ = self.c_attn(x)
        q, k, v  = qkv.split(self.n_embd // mpu.get_tensor_model_parallel_world_size(), dim=2)

        q = q.view(B, T, self.n_head_per_partition, C_partitioned // self.n_head_per_partition)
        k = k.view(B, T, self.n_head_per_partition, C_partitioned // self.n_head_per_partition)
        v = v.view(B, T, self.n_head_per_partition, C_partitioned // self.n_head_per_partition)
        ...
        y = y.transpose(1, 2).contiguous().view(B, T, C_partitioned) # re-assemble all head outputs side by side
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_fc = tensor_parallel.ColumnParallelLinear(
            config.n_embd, 4 * config.n_embd, bias=True, skip_bias_add=True, gather_output=False)
        self.c_proj = tensor_parallel.RowParallelLinear(
            4 * config.n_embd, config.n_embd, bias=True, skip_bias_add=False, input_is_parallel=True)
        self.dropout = nn.Dropout(config.dropout)

    def forward(self, x):
        x_parallel, bias_parallel = self.c_fc(x)
        x_parallel = bias_gelu_fn(bias_parallel, x_parallel)
        x_reduced, _ = self.c_proj(x_parallel)
        x = self.dropout(x_reduced)
        return x
